 ...
 */
static void rx600_serial_setmode(const SIOPINIB* p_siopinib, uint8_t bitrate, uint8_t clksrc)
{
	uint8_t i;
 ...
 *  SIOドライバの初期化ルーチン
 */
void rx600_serial_initialize(void)
{
	SIOPCB* p_siopcb;
	uint_t  i;
 ...
 *  カーネル起動時のバナー出力用の初期化
 */
void rx600_serial_init(ID siopid, uint8_t bitrate, uint8_t clksrc)
{
	...
	/*  ハードウェアの初期化処理と送信許可  */
	rx600_serial_setmode(p_siopinib, bitrate, clksrc);
	sil_wrb_mem((void*)p_siopinib->ctlreg,
 ...
 */
void rx600_serial_pol_putc(char_t c, ID siopid)
{
	 ...
	 *  送信レジスタが空になるまで待つ
	 */
	while ((sil_reb_mem(( void* )p_siopinib->ssrreg) & SCI_SSR_TEND_BIT) == 0U);
 ...
 *  シリアルI/Oポートのオープン
 */
SIOPCB* rx600_serial_opn_por(ID siopid, intptr_t exinf, uint8_t bitrate, uint8_t clksrc)
{
	SIOPCB* p_siopcb;
	const   SIOPINIB* p_siopinib;
 ...
 *  シリアルI/Oポートのクローズ
 */
void rx600_serial_cls_por(SIOPCB* p_siopcb)
{
	/*
	 *  SERIAL停止
	 */
 ...
 *  シリアルI/Oポートへの文字送信
 */
bool_t rx600_serial_snd_chr(SIOPCB* p_siopcb, char_t c)
{
	bool_t ercd = false;
 ...
 *  シリアルI/Oポートからの文字受信
 */
int_t rx600_serial_rcv_chr(SIOPCB* p_siopcb)
{
	int_t c = -1;
 ...
 */
void rx600_serial_ena_cbr(SIOPCB* p_siopcb, uint_t cbrtn)
{
	switch (cbrtn)
	{
		case SIO_RDY_SND:
 ...
 */
void rx600_serial_dis_cbr(SIOPCB* p_siopcb, uint_t cbrtn)
{
	switch (cbrtn)
	{
		case SIO_RDY_SND:
 ...
 *  SIOの割込みサービスルーチン
 */
void rx600_serial_tx_isr(ID siopid)
{
	SIOPCB* p_siopcb = get_siopcb(siopid);

	if ((sil_reb_mem((void*)p_siopcb->p_siopinib->ssrreg) & SCI_SSR_TEND_BIT) != 0U)
	{
		/*
		 *  送信可能コールバックルーチンを呼び出す．
		 */
		rx600_serial_irdy_snd(p_siopcb->exinf);
	}
}

void rx600_serial_rx_isr(ID siopid)
{
	SIOPCB* p_siopcb = get_siopcb(siopid);
	 ...
	 *  受信通知コールバックルーチンを呼び出す．
	 */
	rx600_serial_irdy_rcv(p_siopcb->exinf);
}
