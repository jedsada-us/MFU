...
void serial_initialize(intptr_t exinf)
{
	...
	for (p_spcb = spcb_table, i = 0; i < TNUM_SIOP; p_spcb++, i++)
	{
 ...
 */
ER_UINT serial_wri_dat(ID portid, const char_t* buf, uint_t len)
{
	SPCB* p_spcb;
	bool_t buffer_full;
	uint_t wricnt = 0U;
...
static bool_t serial_rea_chr(SPCB* p_spcb, char_t* p_c)
{
	...
	*p_c = p_spcb->p_spinib->rcv_buffer[p_spcb->rcv_read_ptr];
	 ...
	 *  STARTを送信する．
	 */
	if (p_spcb->rcv_stopped && p_spcb->rcv_count <= BUFCNT_START(p_spcb->p_spinib->rcv_bufsz))
	{
...
bool_t serial_get_chr(ID portid, char_t* p_c)
{
	...
	if (1 <= portid && portid <= TNUM_SIOP) /* ポート番号のチェック */
	{
		p_spcb = get_spcb(portid);
		if (p_spcb->openflag) /* オープン済みかのチェック */
		{
